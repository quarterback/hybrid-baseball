...
| Required Run Rate /3 Outs | RRR/3O | Cricket-over analog: runs per 3 outs the second-batting side still needs to win (~1.3 league pace). Also drives the chasing-side manager AI (`RRR_MANAGER_ENABLED`): best-bat deployment + swing-for-fences when behind pace, premium-asset preservation when the chase is dead. | `REQ_RR × 3` = `(target − scored) / outs_remaining × 3` | `o27/stats/team.py:required_run_rate_3o`; engine `o27/engine/state.py:chase_rrr_3o`, `o27/engine/manager.py:rrr_*` |
