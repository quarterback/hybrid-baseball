...
**A 27-out half is a relay, not a solo.** Nobody goes the distance — the continuous arc that never resets is *too* punishing for one arm, so every half is carried by a procession of pitchers (typically around six a side). Stamina is still the most valuable pitching attribute, but its payoff is now *length*, not the complete game: a high-stamina starter buys more outs before the cascade begins and so needs fewer arms behind him, while a stamina-poor staff churns through openers and bulk relievers faster. How long any given pitcher lasts is emergent — his stamina sets the fatigue budget, grit and a low release slow the decline, and the manager pulls him once he tires or has turned the order — not a fixed pitch count. Career arcs are still longer than MLB's because sidearm/submarine deliveries are less stressful — the durable veteran who eats the most outs per appearance remains a prize.
...
# Almanac (Fangraphs-style stats explorer):
# Live: served by the o27v2 web app at /almanac — linked from the topbar.
# Data is read from o27v2/o27v2.db with mtime-keyed caching, so it always
# reflects the latest sim without rebuilding anything.
#
# Standalone export (snapshots / offline preview):
make almanac-build                                                  # writes ./site/
make almanac-serve                                                  # build + preview on :8765
python -m o27.almanac build --source season-bundle.json --out site/ # build from a JSON bundle
