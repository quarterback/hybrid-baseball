...
def _db_team_to_engine(
    ...
    for p in players:
        ...
        player = Player(
            ...
            # Bunt skill (bat control). Derive from contact+speed for rows that
            # predate the column (NULL/0) so legacy players bunt sensibly.
            bunt=_scout.to_unit(
                p.get("bunt")
                or round(0.6 * (p.get("contact") or 50) + 0.4 * (p.get("speed") or 50))
            ),
    ...
    for phase in phases:
        for engine_pid, bstat in per_phase.get(phase, {}).items():
            ...
            rows.append({
                ...
                "sh": getattr(bstat, "sh", 0),
                "bunt_att": getattr(bstat, "bunt_att", 0),
                "bunt_hits": getattr(bstat, "bunt_hits", 0),
                "sqz": getattr(bstat, "sqz", 0),
                ...
                "risp_pa":   getattr(bstat, "risp_pa", 0),
                "risp_ab":   getattr(bstat, "risp_ab", 0),
                "risp_h":    getattr(bstat, "risp_h", 0),
                "risp_2b":   getattr(bstat, "risp_2b", 0),
                "risp_3b":   getattr(bstat, "risp_3b", 0),
                "risp_hr":   getattr(bstat, "risp_hr", 0),
                "risp_bb":   getattr(bstat, "risp_bb", 0),
                "risp_hbp":  getattr(bstat, "risp_hbp", 0),
                "risp_rbi":  getattr(bstat, "risp_rbi", 0),
                ...
                "entered_inning": int(getattr(bstat, "entered_inning", 0) or 0),
                "entered_outs": int(getattr(bstat, "entered_outs", 0) or 0),
...
def _extract_pitcher_stats(state: GameState, team_id: int, players: list[dict]) -> list[dict]:
    ...
    for (pid_str, phase), spells in by_phase_pid.items():
        ...
        # Finisher stats, derived per spell from its entry/min lead + finished
        # flag, then summed to the game row (season totals are SUMs). A spell
        # "finished" means the pitcher was on the mound at the end of his
        # defensive half. Terminal Outs = outs in a spell where he entered with
        # a lead, never let it be tied/lost, and finished. Quality Finish =
        # finished 9+ outs never trailing. Lead entries/held drive LR%.
        terminal_outs = quality_finish = lead_entries = lead_held = 0
        for rec in spells:
            e_lead = getattr(rec, "entry_lead", 0)
            m_lead = getattr(rec, "min_lead", 0)
            fin    = getattr(rec, "finished", 0)
            outs_r = getattr(rec, "outs_recorded", 0)
            if e_lead > 0:
                lead_entries += 1
                if m_lead > 0:
                    lead_held += 1
        ...
        for r in away_bstats + home_bstats:
            conn.execute(
                ...
                (game_id, r["team_id"], r["player_id"], r["phase"],
                 ...
                 r.get("risp_pa", 0), r.get("risp_ab", 0), r.get("risp_h", 0),
                 r.get("risp_2b", 0), r.get("risp_3b", 0), r.get("risp_hr", 0),
                 r.get("risp_bb", 0), r.get("risp_hbp", 0), r.get("risp_rbi", 0),
                 r.get("game_position", ""),
                 r.get("entry_type", "starter"),
                 r.get("replaced_player_id"),
                 r.get("entered_inning", 0),
                 r.get("entered_outs", 0),
            ...
            conn.executemany(
                """INSERT INTO game_pa_log
                   (game_id, team_id, batter_id, pitcher_id, phase, ab_seq, swing_idx,
                    choice, quality, hit_type, pitch_type, fielder_id,
                ...
                [(game_id, role_to_db.get(e["team_id"], None),
                  ...
                  e.get("pitch_type"),
                  int(e["fielder_id"]) if e.get("fielder_id") is not None else None,
        ...
        for r in away_pstats + home_pstats:
            conn.execute(
                ...
                (game_id, r["team_id"], r["player_id"], r["phase"],
                 ...
                 r.get("terminal_outs", 0), r.get("quality_finish", 0),
                 r.get("lead_entries", 0), r.get("lead_held", 0)),
            )
