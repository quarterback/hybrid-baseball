...
GLOSSARY_SECTIONS: list[dict] = [
    ...
    },
    {
        ...
        "entries": [
            ...
            {"key": "decay", "abbr": "Decay", "name": "Late-Arc Decay (Fatigue)",
             "desc": "The FATIGUE/arc axis: drift-corrected fade in K% from arc-1 (outs 1-9) to arc-3 (outs 19-27). 0 = matches league norm; positive = fades worse; negative = holds up better. Measures how the long arc wears a pitcher down. The separate FAMILIARITY axis — how a lineup times him up across repeat looks — is the Deception grade + Times-Through-the-Order splits (almanac). Lower is better."},
            {"key": "deception", "abbr": "Dec", "name": "Deception Grade",
             "desc": "20-80 scouting grade for how un-timeable a pitcher's arsenal stays across a 27-out arc, from the repertoire-weighted timing-resistance of his pitches. Knuckle / eephus / softball-junk arms grade high (un-timeable); pure-velocity arms grade low (figured out by the 4th look). 50 = league-neutral. The familiarity-axis complement to Decay's fatigue axis."},
            {"key": "gb_pct", "abbr": "GB%", "name": "Groundball Rate",
             "desc": "Share of balls in play hit on the ground (launch angle < 10°). Now genuinely pitch-driven via per-pitch launch-angle bias — sinkers, peeled drops and drop knucks run high; riseballs and rise knucks run low. Paired with LD%/FB% and GO/AO (groundout-to-airout ratio) as the batted-ball profile."},
            {"key": "tto_k_decay", "abbr": "TTOk", "name": "Times-Through K-Decay",
             "desc": "K% points lost from the 1st time facing a hitter in a game to the 3rd+ time, from engine-side times-through-the-order counters (the FAMILIARITY axis, sibling to Decay's fatigue axis). Positive = the lineup figures him out across repeat looks; ~0 / negative = he holds his whiffs as familiarity grows (the deception arbitrage). A small-sample split — meaningful in aggregate / over a full season."},
            {"key": "tto_hardhit_delta", "abbr": "TTOΔ", "name": "Times-Through Hard-Hit Δ",
             "desc": "The contact-quality companion to TTO K-Decay: change in hard-hit% allowed from the 1st to the 3rd+ look (from the batted-ball log). Positive = hitters square him up more with familiarity."},
