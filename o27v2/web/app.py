...
# rate toward the league mean with a prior worth one arc (9 outs) of
# league-average ball, so partial outings regress to the mean instead of
# exploding. A league-rate pitcher returns exactly league wERA at any
...
def _league_dips_constant() -> float:
    ...
    lg_raw = (
        13.0 * (row.get("hr") or 0)
        + 3.0 * ((row.get("bb") or 0) + (row.get("hbp") or 0))
        - 2.0 * ((row.get("k") or 0) + (row.get("fo") or 0))
    ) / ip
    return lg_era - lg_raw
...
def _pitcher_fop(
    ...
    """
    ip = outs / 3.0
    if ip <= 0 or league_era <= 0:
        return 50.0
    dips_era = (
        13.0 * hr + 3.0 * (bb + hbp) - 2.0 * (k + fo)
    ) / ip + dips_const
    if dips_era <= 0:
...
def _aggregate_pitcher_rows(
    ...
    # the legacy rate.
    league_wera_prior = (baselines.get("league_werra")
                         or baselines.get("era") or 0.0)
    league_era_ref = league_wera_prior
    ...
    for p in rows:
        ...
        # Game Equivalents — total workload in complete-game-worths
        # (one full O27 game == 27 outs). The Out-Share-native synthetic
        # workload number: a starter who threw 240 outs logged ~8.9 game
        # equivalents. Synthetic IP, if ever surfaced, is just GE × 9.
        p["game_equiv"] = (outs / 27.0)
...
_O27I_PITCHER_METRICS = [
    ...
    ("k_pct",          "Strikeout %",    "pct", False),
    ("bb_pct",         "Walk %",         "pct", True),
    ("hr_pct",         "HR %",           "pct", True),
]
...
def _o27i_pitcher_rows(team_ids, min_bip: int) -> list[dict]:
    ...
    for e in ev:
        ...
        rows.append({
            ...
            "hardhit":        round(100 * e["hardhit"], 1),
            "barrel":         round(100 * e["barrel"], 1),
            "k_pct":          round(100 * (rt.get("k") or 0) / bf, 1) if bf else None,
            "bb_pct":         round(100 * (rt.get("bb") or 0) / bf, 1) if bf else None,
            "hr_pct":         round(100 * (rt.get("hr") or 0) / bf, 1) if bf else None,
        })
    if rows:
        ids = ",".join(str(r["player_id"]) for r in rows)
        names = {n["id"]: n for n in db.fetchall(
            f"""SELECT p.id, p.name, t.abbrev AS team_abbrev
                FROM players p LEFT JOIN teams t ON t.id = p.team_id
                WHERE p.id IN ({ids})""")}
        for r in rows:
...
def player_o27i(player_id: int):
    ...
    def _build_sliders(me, metrics):
        out = []
        if not me:
            return out
        for key, label, fmt, _rev in metrics:
            val = me.get(key)
...
def _universe_style_options():
    ...
    keys = [
        "balanced", "mlb",
        # Talent styles — how the generated PLAYERS come out (see _STYLE_PROFILES).
        "sluggers", "pure_hitters", "on_base", "speedsters", "defenders",
        "power_arms", "finesse_arms", "five_tool", "polished",
        "hitting_rich", "pitching_rich",
        # Regional flavor profiles.
        "npb", "dominican", "european", "caribbean", "athletic",
    ]
