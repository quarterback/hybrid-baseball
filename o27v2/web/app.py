...
# Career rate-stat qualification (totals across all archived seasons). Kept
# modest so short histories still populate the rate leaderboards.
_CAREER_MIN_PA   = 50
_CAREER_MIN_OUTS = 60


def _career_meta(table: str) -> dict[int, dict]:
    """Latest name / team / league per player, from their most recent
    archived season line in `table`."""
    rows = db.fetchall(
        f"""SELECT player_id, player_name, team_abbrev, league FROM {table} b
             WHERE season_id = (SELECT MAX(season_id) FROM {table}
                                 WHERE player_id = b.player_id)"""
    )
    return {r["player_id"]: r for r in rows}


def _career_batting() -> list[dict]:
    rows = db.fetchall(
        """SELECT player_id, COUNT(DISTINCT season_id) AS seasons,
                  SUM(g) AS g, SUM(pa) AS pa, SUM(ab) AS ab, SUM(r) AS r,
                  SUM(h) AS h, SUM(doubles) AS d2, SUM(triples) AS d3,
                  SUM(hr) AS hr, SUM(rbi) AS rbi, SUM(bb) AS bb,
                  SUM(k) AS k, SUM(sb) AS sb, SUM(hbp) AS hbp
             FROM season_player_batting GROUP BY player_id"""
    )
    meta = _career_meta("season_player_batting")
    out: list[dict] = []
    for r in rows:
        d = dict(r)
        m = meta.get(r["player_id"], {})
        d["player_name"] = m["player_name"] if m else "?"
        d["team_abbrev"] = m["team_abbrev"] if m else ""
        d["league"]      = m["league"] if m else ""
        ab = r["ab"] or 0; h = r["h"] or 0; bb = r["bb"] or 0; hbp = r["hbp"] or 0
        d2 = r["d2"] or 0; d3 = r["d3"] or 0; hr = r["hr"] or 0
        tb = (h - d2 - d3 - hr) + 2 * d2 + 3 * d3 + 4 * hr
        d["tb"]  = tb
        d["avg"] = h / ab if ab else 0.0
        obp_den  = ab + bb + hbp
        d["obp"] = (h + bb + hbp) / obp_den if obp_den else 0.0
        d["slg"] = tb / ab if ab else 0.0
        d["ops"] = d["obp"] + d["slg"]
        out.append(d)
    return out


def _career_pitching() -> list[dict]:
    rows = db.fetchall(
        """SELECT player_id, COUNT(DISTINCT season_id) AS seasons,
                  SUM(g) AS g, SUM(gs) AS gs, SUM(w) AS w, SUM(l) AS l,
                  SUM(outs) AS outs, SUM(h) AS h, SUM(r) AS r, SUM(er) AS er,
                  SUM(bb) AS bb, SUM(k) AS k, SUM(hr) AS hr
             FROM season_player_pitching GROUP BY player_id"""
    )
    meta = _career_meta("season_player_pitching")
    out: list[dict] = []
    for r in rows:
        d = dict(r)
        m = meta.get(r["player_id"], {})
        ...
        ip = outs / 3.0
        d["ip"] = ip
        d["ip_disp"] = f"{outs // 3}.{outs % 3}"
        d["era"]  = (r["er"] or 0) * 27.0 / outs if outs else 0.0
        d["whip"] = ((r["bb"] or 0) + (r["h"] or 0)) / ip if ip else 0.0
        d["k9"]   = (r["k"] or 0) * 27.0 / outs if outs else 0.0
        out.append(d)
    return out


def _rank(rows, key, *, reverse=True, n=20, min_field=None, min_val=0):
    pool = [r for r in rows if (min_field is None or (r.get(min_field) or 0) >= min_val)]
    return sorted(pool, key=lambda r: (r.get(key) or 0), reverse=reverse)[:n]


@app.route("/seasons/career")
def seasons_career():
    bat = _career_batting()
    pit = _career_pitching()
    has_data = bool(bat or pit)
    n_seasons = db.fetchone(
    ...
    batting = {
        ...
        "sb":  _rank(bat, "sb"),
        "bb":  _rank(bat, "bb"),
        "avg": _rank(bat, "avg", min_field="pa", min_val=_CAREER_MIN_PA),
        "obp": _rank(bat, "obp", min_field="pa", min_val=_CAREER_MIN_PA),
        "ops": _rank(bat, "ops", min_field="pa", min_val=_CAREER_MIN_PA),
    }
    pitching = {
        "w":    _rank(pit, "w"),
        "k":    _rank(pit, "k"),
        "g":    _rank(pit, "g"),
        "gs":   _rank(pit, "gs"),
        "era":  _rank(pit, "era",  reverse=False, min_field="outs", min_val=_CAREER_MIN_OUTS),
        "whip": _rank(pit, "whip", reverse=False, min_field="outs", min_val=_CAREER_MIN_OUTS),
        "k9":   _rank(pit, "k9",   min_field="outs", min_val=_CAREER_MIN_OUTS),
    }
    return _serve("career.html", batting=batting, pitching=pitching,
                  has_data=has_data, n_seasons=n_seasons,
                  min_pa=_CAREER_MIN_PA, min_outs=_CAREER_MIN_OUTS)
