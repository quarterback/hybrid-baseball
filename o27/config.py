...
PITCHER_CHANGE_BASE: int  = 4
PITCHER_CHANGE_SCALE: int = 6

# Modern cascading-staff usage — NO pitcher rides a single half by himself.
# Starts run roughly HALF the old pre-1980s targets: a classical SP turns the
# order ~once (target AOR ~5-7 outs) and even a workhorse tops out well short
# of a complete game. Length still emerges from ratings (stamina sets the role
# tier, pitcher_skill scales the leash, and the fatigue model below can pull a
# gassed arm earlier) — it is not a flat pitch cap. With reach rate ~33%, a
# ~16 BF workhorse threshold ≈ 11 outs, so the deepest aces are pulled around
# the middle of the half and the bullpen cascades the rest.
WORKHORSE_CHANGE_BASE: int   = 6    # workhorse starter still goes deepest…
WORKHORSE_CHANGE_SCALE: int  = 4    # …but at skill 0.5 → ~8 BF (~5 outs), not 22
...
# Dedicated relievers — short cascading stints (AOR ~3-4 outs) so a single
# half is carried by a procession of arms, not one or two. They no longer wait
# for a fixed out count to enter; the starter is pulled on his own threshold /
# fatigue and the pen takes over from there.
RELIEVER_CHANGE_BASE: int    = 6    # short-relief stints
RELIEVER_CHANGE_SCALE: int   = 3
# DEPRECATED: the old "don't pull the SP for an RP before out N" guard that
# forced workhorses to ride toward a complete game. It is no longer consulted
# by should_change_pitcher — starters now come out on their threshold/fatigue
# whenever it triggers. Kept only so legacy imports/saves don't break.
RELIEVER_ENTRY_OUTS_MIN: int = 18
...
OPENER_CHANGE_BASE:  int = 4
OPENER_CHANGE_SCALE: int = 2

# Fatigue-reactive hook — independent of batters-faced. Once the pitcher's
# consecutive-pitch fatigue (see prob.pitch_fatigue_level: stamina sets the
# budget, grit/release slow it, weather speeds it) crosses this tolerance, the
# skipper goes to the bullpen even if the BF threshold above hasn't been hit.
# This is what makes a long-count / high-stress spell end early, and it scales
# with skill so a better arm earns a slightly longer leash before the hook.
# It is a *reaction to fatigue*, not a fixed pitch cap.
PULL_FATIGUE_TOLERANCE:   float = 0.10  # base gas-level that triggers a pull
PULL_FATIGUE_SKILL_LEASH: float = 0.18  # added tolerance at pitcher_skill 1.0
